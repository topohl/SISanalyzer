...
# Read data
anxiety_data <- read_excel("S:/Lab_Member/Tobi/Experiments/Exp9_Social-Stress/Analysis/SIS_Analysis/E9_Behavior_Data.xlsx", sheet = "anxiety score")
...
con_norm <- shapiro.test(anxiety_data$CombZ[means$Group == "CON"])
con_norm

# Normality test for RES group
res_norm <- shapiro.test(anxiety_data$CombZ[means$Group == "RES"])
res_norm

# Normality test for SUS group
sus_norm <- shapiro.test(anxiety_data$CombZ[means$Group == "SUS"])
sus_norm
...
anxiety_data %>%
  ...
  labs(title = bquote(~bold("zscore4")),
       subtitle = "(anxiety-related behavior)",
       caption = "",
       x = NULL,
       y = "z score [a.u.]") +
...
# use SUS animals 1 SD below depressive score
