...
# Read data
cognition_data <- read_excel("S:/Lab_Member/Tobi/Experiments/Exp9_Social-Stress/Analysis/SIS_Analysis/E9_Behavior_Data.xlsx", sheet = "cognition score")
...
means <- cognition_data %>% 
  group_by(Group) %>% 
  summarise(mean = mean(CombZ),
            sd_CombZ = sd(CombZ))

# Normality test for CTRL group
con_norm <- shapiro.test(cognition_data$CombZ[means$Group == "CON"])
con_norm

# Normality test for RES group
res_norm <- shapiro.test(cognition_data$CombZ[means$Group == "RES"])
res_norm

# Normality test for SUS group
sus_norm <- shapiro.test(cognition_data$CombZ[means$Group == "SUS"])
sus_norm
...
cognition_data %>%
  ...
  labs(title = bquote(~bold("zscore5")),
       subtitle = "(cognition)",
       caption = "",
       x = NULL,
       y = "z score [a.u.]") +
...
# use animals identified 1 SD below depressive score
